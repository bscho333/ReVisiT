...
class GenerationMixin:
    ...
    def generate(
        ...
        **kwargs,
        ...
        if generation_config.use_revisit:
            return self.revisit_decoding(
                input_ids,
                image_tokens=generation_config.image_tokens,
    ...
    def revisit_decoding(
        ...
        **model_kwargs,
        ...
        if image_tokens is None:
            image_token_id = -200
            image_token_position_in_input = int(torch.where(input_ids == image_token_id)[-1])
            image_tokens = torch.arange(image_token_position_in_input, image_token_position_in_input + 576, device=input_ids.device)
        ...
        if not self.config.tie_word_embeddings:
            ...
            start_layer = 0
        if isinstance(early_exit_layers, str) and early_exit_layers == "all":
            if final_layer % 2 == 0:
                early_exit_layers = list(range(start_layer, final_layer + 1, 2))
            else:
                early_exit_layers = list(range(start_layer, final_layer + 1, 2)) + [final_layer]
        elif isinstance(early_exit_layers, str) and early_exit_layers == "last":
            early_exit_layers = [final_layer]
        elif isinstance(early_exit_layers, str) and early_exit_layers == "low":
            if start_layer == final_layer // 2:
                early_exit_layers = [start_layer]
            else:
                early_exit_layers = (
            ...
            early_exit_layers.append(final_layer)
        elif isinstance(early_exit_layers, str) and early_exit_layers == "high":
            early_exit_layers = (
        ...
        elif isinstance(early_exit_layers, list):
            early_exit_layers = [i for i in early_exit_layers if i < final_layer]
        else:
            raise ValueError("early_exit_layers must be either 'all', 'last', 'low', 'high', or a list of integers.")
        ...
        while True:
            ...
            if first_timestep:
                ...
                stacked_candidate_layers = [lm_head(outputs.hidden_states[i][:, image_tokens, :]) for i in early_exit_layers]
                stacked_candidate_layers = torch.stack(stacked_candidate_layers, dim=1)
                stacked_candidate_layers = stacked_candidate_layers.view(
                    stacked_candidate_layers.size(0), -1, stacked_candidate_layers.size(-1)
                )
            ...
            final_logits = _relative_top_filter(final_logits, None, relative_top)
            mask = final_logits[0] < -1e3
            masked = final_logits[0] > -1e3


            stacked_premature_layers = stacked_candidate_layers[:, :, masked]

            softmax_premature_layers = F.softmax(stacked_premature_layers, dim=-1)
            log_softmax_premature_layers = F.log_softmax(stacked_premature_layers, dim=-1)

            softmax_mature_layer = F.softmax(final_logits[:, masked], dim=-1)
            log_softmax_mature_layer = F.log_softmax(final_logits[:, masked], dim=-1)
            ...
            js_divs = 0.5 * (kl1 + kl2)
            js_divs = js_divs.mean(0)
            idx = int(js_divs.argmin().cpu().item())
            base_logits = stacked_candidate_layers[:, idx, :]
            base_logits = base_logits.log_softmax(dim=-1)
            base_logits[0][mask] = -1e3
            next_token_logits = final_logits
            base_candidate_logits = log_softmax_premature_layers[0][idx]
            final_candidate_logits = log_softmax_mature_layer[0]
            next_token_logits[0][masked] = final_candidate_logits + base_candidate_logits
