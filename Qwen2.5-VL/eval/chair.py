...
def save_hallucinated_words(cap_file, cap_dict): 
    ...
    for i in cap_dict["sentences"]:
        if len(i['mscoco_hallucinated_words']) > 0:
            hallucinated_results.append(i)
    with open(cap_file, 'w') as f:
        json.dump(cap_dict, f, indent=2, ensure_ascii=False)
    with open(cap_file.replace('result.jsonl', 'hallucinated_result.jsonl'), 'w') as f:
        json.dump(hallucinated_results, f, indent=2, ensure_ascii=False)
